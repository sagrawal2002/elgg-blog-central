...
function trackback_init()
	...
	function trackback_page_handler($page) 
	{
		...
		if (isset($page[0])) {
			$username = $page[0];
			$security_hash = $page[1];

			if(substr(md5(md5($username).get_site_secret()),5,9) == $security_hash) {
				set_input('username',$page[0]);
			} else {
				set_input('username',false);				
			}
		}
...
function blogextended_blog_type_handler($event, $object_type, $object){
  ...
  include dirname(__FILE__) . '/lib/blog_lib.php';
	...
	// Register actions
		...
		register_action("blog/featured",false,$CONFIG->pluginspath . "blog/actions/featured.php");		
		register_action("blog/watch",false,$CONFIG->pluginspath . "blog/actions/watch.php");
